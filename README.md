...
We have tags for each Spark version starting `1.6.0` to `2.3.1`. The tags `1` and `1.6` point to the `1.6.3` which is the latest release for that major/minor version. Similarly `2` and `2.3` point to `2.3.1`. `latest` always points to latest Spark release which in this case is `2.3.1`.
